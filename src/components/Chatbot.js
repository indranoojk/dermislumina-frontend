...
const Chatbot = () => {
  const [messages, setMessages] = useState([
    'Hello! I\'m your skincare assistant. Please enter "hi" or "hello" to start.'
  ]);
  const [inputValue, setInputValue] = useState('');
  const [isChatbotOpen, setIsChatbotOpen] = useState(false);
  const [isGreetingEntered, setIsGreetingEntered] = useState(false); // New state for greeting check
  const messagesRef = useRef(null);

  const handleSendMessage = () => {
    if (!isGreetingEntered) {
      // Check for greeting
      if (inputValue.toLowerCase() === 'hi' || inputValue.toLowerCase() === 'hello') {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          'Please enter "hi" or "hello" to start.',
        ]);
      }
      setInputValue('');
    ...
    // Process user input if greeting is entered
    const response =
      responses[inputValue.toLowerCase()] || 'Sorry, I didn\'t understand that. Can you please rephrase?';
