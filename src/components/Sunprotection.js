        ...
        <h1 className="text-5xl p-4 mb-6 font-medium font-serif">Skin care Assessment</h1>
            ...
            <div className="lg:w-96 grid gap-5 p-2 lg:ml-[33%] -mt-10 lg:mt-0 top-16 rounded-2xl absolute ml-8 mr-8">
                  <h3 className='text-4xl font-bold text-white'>What's your approach to sun protection?</h3>
                    <Link to='/sleeptime' className={`px-2 py-3 bg-[#eb9ce5] cursor-pointer text-white lg:text-black rounded-md text-xl hover:bg-[#fffefe] onClick={handleClick}  ${selectedSunProtection === 'Never leave the house without SPF'? 'bg-white text-purple-600' : ''}`} value="Never leave the house without SPF" name='sunprotection' onClick={() => handleOptionClick('Never leave the house without SPF')}><button>Never leave the house without SPF</button></Link>

                    <Link to='/sleeptime' className={`px-2 py-3 bg-[#eb9ce5] cursor-pointer text-white lg:text-black rounded-md text-xl hover:bg-[#fffefe] onClick={handleClick}  ${selectedSunProtection === 'only on sunny days'? 'bg-white text-purple-600' : ''}`} value="only on sunny days" name='sunprotection' onClick={() => handleOptionClick('only on sunny days')}><button>only on sunny days</button></Link>

                    <Link to='/sleeptime' className={`px-2 py-3 bg-[#eb9ce5] cursor-pointer text-white lg:text-black rounded-md text-xl hover:bg-[#fffefe] onClick={handleClick}  ${selectedSunProtection === 'Occasionally'? 'bg-white text-purple-600' : ''}`} value="Occasionally" name='sunprotection' onClick={() => handleOptionClick('Occasionally')}><button>Occasionally</button></Link>

                    <Link to='/sleeptime' className={`px-2 py-3 bg-[#eb9ce5] cursor-pointer text-white lg:text-black rounded-md text-xl hover:bg-[#fffefe] onClick={handleClick}  ${selectedSunProtection === 'Rarely use sunscreen'? 'bg-white text-purple-600' : ''}`} value="Rarely use sunscreen" name='sunprotection' onClick={() => handleOptionClick('Rarely use sunscreen')}><button >Rarely use sunscreen</button></Link>
